   ...
   public void publish(StringBuffer var1) {
      ...
      if (this.bookmarks.size() != 0) {
         ...
         while(var2.hasNext()) {
            ...
            if (null == var6 && var5.size() > 0) {
               var6 = this.references.get(var5.get(0));
            }

            var1.append("\t<fo:bookmark internal-destination=\"" + var6 + "\">\n");
            var1.append("\t\t<fo:bookmark-title>" + Content.fixText(var4) + "</fo:bookmark-title>\n");
            Iterator var7 = var5.iterator();

            while(var7.hasNext()) {
               String var8 = (String)var7.next();
               var1.append("\t\t<fo:bookmark internal-destination=\"" + this.references.get(var8) + "\">\n");
               var1.append("\t\t\t<fo:bookmark-title>" + Content.fixText(var8) + "</fo:bookmark-title>\n");
