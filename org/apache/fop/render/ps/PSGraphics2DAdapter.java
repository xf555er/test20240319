...
public class PSGraphics2DAdapter extends AbstractGraphics2DAdapter {
   ...
   public void paintImage(Graphics2DImagePainter painter, RendererContext context, int x, int y, int width, int height) throws IOException {
      ...
      PSGraphics2D graphics = painter instanceof GeneralGraphics2DImagePainter ? (PSGraphics2D)((GeneralGraphics2DImagePainter)painter).getGraphics(false, this.gen) : new PSGraphics2D(false, this.gen);
      ...
      if (paintAsBitmap) {
         ...
         if (painter instanceof GeneralGraphics2DImagePainter) {
            PSFontUtils.addFallbackFonts(this.fontInfo, (GeneralGraphics2DImagePainter)painter);
         }
