...
public class PSRenderingUtil {
   ...
   PSRenderingUtil(FOUserAgent userAgent) {
      this.renderingMode = PSRenderingMode.QUALITY;
      this.userAgent = userAgent;
      this.initialize();
   }

   private void initialize() {
      Object obj = this.userAgent.getRendererOptions().get(PSRendererOption.AUTO_ROTATE_LANDSCAPE.getName());
      if (obj != null) {
         this.setAutoRotateLandscape(this.booleanValueOf(obj));
      }

      obj = this.userAgent.getRendererOptions().get(PSRendererOption.LANGUAGE_LEVEL.getName());
      if (obj != null) {
         this.setLanguageLevel(this.intValueOf(obj));
      }

      obj = this.userAgent.getRendererOptions().get(PSRendererOption.OPTIMIZE_RESOURCES.getName());
      if (obj != null) {
         this.setOptimizeResources(this.booleanValueOf(obj));
      }

      obj = this.userAgent.getRendererOptions().get(PSRendererOption.ACROBAT_DOWNSAMPLE.getName());
      if (obj != null) {
         this.setAcrobatDownsample(this.booleanValueOf(obj));
      }
