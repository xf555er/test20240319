...
public class AFPSVGHandler extends AbstractGenericSVGHandler {
   ...
   private AFPObjectAreaInfo createObjectAreaInfo(AFPPaintingState paintingState, int x, int y, int width, int height, int resolution) {
      ...
      int objWidth = Math.round(unitConv.mpt2units((float)width));
      int objHeight = Math.round(unitConv.mpt2units((float)height));
   ...
   public static BridgeContext createBridgeContext(FOUserAgent userAgent, AFPGraphics2D g2d) {
      ...
      SVGUserAgent svgUserAgent = new SVGUserAgent(userAgent, new AggregatingFontFamilyResolver(new FontFamilyResolver[]{new AFPFontFamilyResolver(fontInfo, userAgent.getEventBroadcaster()), DefaultFontFamilyResolver.SINGLETON}), new AffineTransform());
