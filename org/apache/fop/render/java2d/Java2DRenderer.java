...
public abstract class Java2DRenderer extends AbstractPathOrientedRenderer implements Printable {
   ...
   protected void restoreStateStackAfterBreakOut(List breakOutList) {
      ...
      for(Iterator var2 = breakOutList.iterator(); var2.hasNext(); this.state = s) {
         Object aBreakOutList = var2.next();
         s = (Java2DGraphicsState)aBreakOutList;
         this.stateStack.push(this.state);
      }
   ...
   public void renderText(TextArea text) {
      ...
      this.state.transform(at);
      renderText(text, this.state.getGraph(), font, this.fontInfo);
      this.restoreGraphicsState();
   ...
   public static void renderText(TextArea text, Graphics2D g2d, Font font, FontInfo fontInfo) {
      ...
      Iterator var6 = text.getChildAreas().iterator();

      while(true) {
         while(var6.hasNext()) {
            Object child = var6.next();
            String s;
