...
public class RtfListTable extends RtfContainer {
   ...
   public void writeRtfContent() throws IOException {
      ...
      if (this.lists != null) {
         ...
         while(var1.hasNext()) {
            Object list1 = var1.next();
            RtfList list = (RtfList)list1;
         ...
         int z = 1;
         this.newLine();
         Iterator var6 = this.styles.iterator();

         while(var6.hasNext()) {
            Object style1 = var6.next();
            RtfListStyle style = (RtfListStyle)style1;
            this.writeGroupMark(true);
            this.writeStarControlWordNS("listoverride");
            this.writeGroupMark(true);
            this.writeOneAttributeNS("listid", style.getRtfList().getListId().toString());
            this.writeOneAttributeNS("listoverridecount", 0);
            this.writeOneAttributeNS("ls", z++);
            this.writeGroupMark(false);
            this.writeGroupMark(false);
            this.newLine();
         }
   ...
   private void writeListTableEntry(RtfList list) throws IOException {
      ...
      this.writeOneAttributeNS("levelspace", 0);
