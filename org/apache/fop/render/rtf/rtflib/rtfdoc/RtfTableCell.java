...
public class RtfTableCell extends RtfContainer implements IRtfParagraphContainer, IRtfListContainer, IRtfTableContainer, IRtfExternalGraphicContainer, IRtfTextrunContainer {
   ...
   int writeCellDef(int offset) throws IOException {
      if (this.hMerge == 2) {
         ...
         if (this.attrib.getValue("number-columns-spanned") != null) {
            nbMergedCells = (Integer)this.attrib.getValue("number-columns-spanned");
         ...
         nbMergedCells = offset + iCurrentWidth;
         if (this.setCenter) {
         ...
         this.writeControlWord("cellx" + nbMergedCells);
         return nbMergedCells;
      }
   }
   ...
   public void finish() {
      if (this.getRow().getTable().isNestedTable() && this.table != null) {
         ...
         this.lastBreak.switchControlWord(this.getRow().getTable().isNestedTable() ? "nestcell" : "cell");
      }
