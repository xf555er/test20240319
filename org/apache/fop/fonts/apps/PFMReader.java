...
public class PFMReader extends AbstractFontReader {
   ...
   public static void main(String[] args) {
      ...
      if (arguments.length == 2 && options.get("-h") == null && options.get("-help") == null && options.get("--help") == null) {
         try {
            ...
            log.error("Error while building XML font metrics file", var10);
            System.exit(-1);
         }
      } else {
         displayUsage();
   ...
   public Document constructFontXML(PFMFile pfm, String fontName, String className, String resource, String file) {
      ...
      try {
         ...
         log.error("Can't create DOM implementation", var23);
         return null;
      }
      ...
      root.appendChild(el);
      value = Integer.valueOf(pfm.getFirstChar());
      el.appendChild(doc.createTextNode(value.toString()));
      el = doc.createElement("last-char");
      root.appendChild(el);
      value = Integer.valueOf(pfm.getLastChar());
      el.appendChild(doc.createTextNode(value.toString()));
      ...
      Iterator var26 = pfm.getKerning().keySet().iterator();

      while(var26.hasNext()) {
         Object kpx1 = var26.next();
         el = doc.createElement("kerning");
         el.setAttribute("kpx1", kpx1.toString());
         root.appendChild(el);
         Element el2 = null;
         Map h2 = (Map)pfm.getKerning().get(kpx1);
         Iterator var18 = h2.entrySet().iterator();
