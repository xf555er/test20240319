...
public class ImageLayout implements Constants {
   ...
   protected void doLayout() {
      ...
      if (len.getEnum() != 9) {
         bpd = this.evaluateLength(len, this.intrinsicSize.height);
      }
   ...
   private int constrainExtent(int extent, LengthRangeProperty range, Length contextExtent) {
      boolean mayScaleUp = contextExtent.getEnum() != 185;
      boolean mayScaleDown = contextExtent.getEnum() != 186;
      Length len = range.getMaximum(this.percentBaseContext).getLength();
      int min;
      if (len.getEnum() != 9) {
         min = this.evaluateLength(len);
   ...
   private int evaluateLength(Length length, int referenceValue) {
      double numericValue = length.getNumericValue(this.percentBaseContext);
      int bpd = numericValue < 0.0 ? referenceValue : (int)Math.round(numericValue);
      return bpd;
   }
