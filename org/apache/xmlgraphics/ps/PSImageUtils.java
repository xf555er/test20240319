...
public class PSImageUtils {
   ...
   public static void writeImage(ImageEncoder encoder, Dimension imgDim, String imgDescription, Rectangle2D targetRect, ColorModel colorModel, PSGenerator gen) throws IOException {
      writeImage(encoder, imgDim, imgDescription, targetRect, colorModel, gen, (RenderedImage)null);
   }
   ...
   private static void prepareColorSpace(PSGenerator gen, ColorModel cm) throws IOException {
      if (cm instanceof IndexColorModel) {
         ...
         boolean isDeviceGray = Arrays.equals(reds, blues) && Arrays.equals(blues, greens);
         if (isDeviceGray) {
   ...
   public static void renderBitmapImage(RenderedImage img, float x, float y, float w, float h, PSGenerator gen, Color mask) throws IOException {
