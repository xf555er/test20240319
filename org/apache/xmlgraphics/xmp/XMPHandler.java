         ...
         if ("http://www.w3.org/1999/02/22-rdf-syntax-ns#".equals(uri) && !"value".equals(localName)) {
            if ("li".equals(localName)) {
               ...
               if (struct != null) {
                  ...
                  if (s.length() > 0) {
                     res = atts.getValue("http://www.w3.org/XML/1998/namespace", "lang");
                     if (res != null) {
                        this.getCurrentArray(true).add(s, res);
                     } else {
                        this.getCurrentArray(true).add(s);
                     }
                  } else {
                     res = atts.getValue("http://www.w3.org/1999/02/22-rdf-syntax-ns#", "resource");
                     if (res != null) {
                        try {
                           URI resource = new URI(res);
                           this.getCurrentArray(true).add(resource);
                        } catch (URISyntaxException var12) {
                           throw new SAXException("rdf:resource value is not a well-formed URI", var12);
                        }
                     }
                  }
               }
            ...
            if (this.hasComplexContent()) {
               ...
               name = this.popCurrentPropName();
               res = this.content.toString().trim();
               prop = new XMPProperty(name, res);
               ...
               if (res != null) {
                  try {
                     ...
                     throw new SAXException("rdf:resource value is not a well-formed URI", var11);
                  }
               }
            }
