...
class PNGImage extends SimpleRenderedImage implements PNGConstants {
   ...
   private void parse_IEND_chunk(PNGChunk chunk) throws Exception {
      ...
      for(ztextLen = 0; ztextLen < textLen; ++ztextLen) {
         ...
         if (this.emitProperties) {
            ...
            this.properties.put(key.toLowerCase(Locale.getDefault()), val);
         }
      }
   ...
   private void parse_cHRM_chunk(PNGChunk chunk) {
      if (this.sRGBRenderingIntent == -1) {
         ...
         if (this.emitProperties) {
            this.properties.put("white_point_x", this.chromaticity[0]);
            this.properties.put("white_point_y", this.chromaticity[1]);
            this.properties.put("red_x", this.chromaticity[2]);
            this.properties.put("red_y", this.chromaticity[3]);
            this.properties.put("green_x", this.chromaticity[4]);
            this.properties.put("green_y", this.chromaticity[5]);
            this.properties.put("blue_x", this.chromaticity[6]);
            this.properties.put("blue_y", this.chromaticity[7]);
         }
   ...
   private void parse_gAMA_chunk(PNGChunk chunk) {
      if (this.sRGBRenderingIntent == -1) {
         ...
         if (this.emitProperties) {
            this.properties.put("gamma", this.fileGamma * exp);
         }
   ...
   private void parse_pHYs_chunk(PNGChunk chunk) {
      ...
      if (this.emitProperties) {
         ...
         this.properties.put("pixel_aspect_ratio", (float)this.xPixelsPerUnit / (float)this.yPixelsPerUnit);
      ...
      switch (process) {
         ...
         case 1:
            for(srcX = 0; srcX < width; ++srcX) {
               ...
               for(r = 0; r < this.inputBands; ++r) {
                  g = ps[r];
                  ps[r] = this.gammaLut[g];
               }
         ...
         case 4:
            for(srcX = 0; srcX < width; ++srcX) {
               src.getPixel(srcX, 0, ps);
               r = ps[0];
               pd[0] = this.redPalette[r];
               pd[1] = this.greenPalette[r];
               pd[2] = this.bluePalette[r];
               dst.setPixel(dstX, y, pd);
               dstX += step;
            }
