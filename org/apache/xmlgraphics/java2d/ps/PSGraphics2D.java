...
public class PSGraphics2D extends AbstractGraphics2D {
   ...
   public boolean drawImage(Image img, int x, int y, ImageObserver observer) {
      return this.drawImage(img, x, y, observer, (Color)null);
   }
      ...
      if (width != -1 && height != -1) {
         ...
         if (!g.drawImage(img, 0, 0, observer)) {
            ...
            try {
               ...
               PSImageUtils.renderBitmapImage(buf, (float)x, (float)y, (float)width, (float)height, this.gen, mask);
               this.gen.restoreGraphicsState();
            } catch (IOException var13) {
               this.handleIOException(var13);
            }
   ...
   public void drawRenderedImage(RenderedImage img, AffineTransform xform) {
      ...
      try {
         ...
         Shape imclip = this.getClip();
         this.writeClip(imclip);
         PSImageUtils.renderBitmapImage(img, 0.0F, 0.0F, (float)img.getWidth(), (float)img.getHeight(), this.gen, (Color)null);
         this.gen.restoreGraphicsState();
      } catch (IOException var5) {
         this.handleIOException(var5);
      }
