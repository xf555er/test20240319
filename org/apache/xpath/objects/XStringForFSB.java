...
public class XStringForFSB extends XString {
   ...
   private XStringForFSB(String val) {
      super(val);
      throw new IllegalArgumentException(XSLMessages.createXPATHMessage("ER_FSB_CANNOT_TAKE_STRING", (Object[])null));
   }
   ...
   public XMLString fixWhiteSpace(boolean trimHead, boolean trimTail, boolean doublePunctuationSpaces) {
      ...
      int start;
      for(start = this.m_start; start < end; ++start) {
         char c = fsb.charAt(start);
         if (isSpace(c)) {
            if (!pres) {
               if (' ' != c) {
                  edit = true;
