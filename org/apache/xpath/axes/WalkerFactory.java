...
public class WalkerFactory {
   ...
   private static AxesWalker createDefaultWalker(Compiler compiler, int opPos, WalkingIterator lpi, int analysis) {
      ...
      if (simpleInit) {
         ((AxesWalker)ai).initNodeTest(-1);
      } else {
         int whatToShow = compiler.getWhatToShow(opPos);
         if (0 != (whatToShow & 4163) && whatToShow != -1) {
            ((AxesWalker)ai).initNodeTest(whatToShow, compiler.getStepNS(opPos), compiler.getStepLocalName(opPos));
         } else {
            ((AxesWalker)ai).initNodeTest(whatToShow);
         }
      }
