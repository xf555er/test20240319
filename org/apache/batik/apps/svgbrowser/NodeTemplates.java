...
public class NodeTemplates {
   ...
   private void initializeTemplates() {
      ...
      for(int var4 = 0; var4 < var3; ++var4) {
         Field currentField = var2[var4];

         try {
            if (currentField.getType() == String.class && currentField.getName().endsWith("MemberName")) {
               boolean isAccessible = currentField.isAccessible();
               currentField.setAccessible(true);
               String baseFieldName = currentField.get(this).toString();
               String nodeValue = this.getClass().getField(baseFieldName + "Value").get(this).toString();
               String nodeName = this.getClass().getField(baseFieldName + "Name").get(this).toString();
               short nodeType = (Short)this.getClass().getField(baseFieldName + "Type").get(this);
               String nodeDescription = this.getClass().getField(baseFieldName + "Description").get(this).toString();
               String nodeCategory = this.getClass().getField(baseFieldName + "Category").get(this).toString();
               NodeTemplateDescriptor desc = new NodeTemplateDescriptor(nodeName, nodeValue, nodeType, nodeCategory, nodeDescription);
               this.nodeTemplatesMap.put(desc.getName(), desc);
