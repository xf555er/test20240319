...
public final class SAXImpl extends SAX2DTM2 implements DOMEnhancedForDTM, DOMBuilder {
   ...
   private int[] setupMapping(String[] names, String[] uris, int[] types, int nNames) {
      ...
      for(int i = 0; i < nNames; ++i) {
         int type = this.m_expandedNameTable.getExpandedTypeID(uris[i], names[i], types[i], false);
         result[type] = type;
      }
   ...
   public int getGeneralizedType(String name, boolean searchOnly) {
      ...
      int index = true;
      int index;
      if ((index = name.lastIndexOf(58)) > -1) {
   ...
   public int[] getReverseMapping(String[] names, String[] uris, int[] types) {
      ...
      for(i = 0; i < names.length; ++i) {
         int type = this.m_expandedNameTable.getExpandedTypeID(uris[i], names[i], types[i], true);
         result[i + 14] = type;
      }
   ...
   public void print(int node, int level) {
      switch (this.getNodeType(node)) {
         ...
         default:
            ...
            int child;
            for(child = this.getFirstAttribute(node); child != -1; child = this.getNextAttribute(child)) {
               System.out.print("\n" + this.getNodeName(child) + "=\"" + this.getStringValueX(child) + "\"");
            }

            System.out.print('>');

            for(child = this.getFirstChild(node); child != -1; child = this.getNextSibling(child)) {
   ...
   private final void copy(int node, SerializationHandler handler, boolean isChild) throws TransletException {
      ...
      try {
         ...
         throw new TransletException(var11);
      }
   }
   ...
   public DOM getResultTreeFrag(int initSize, int rtfType, boolean addToManager) {
      ...
      if (rtfType == 0) {
         ...
         return (DOM)this._dtmManager.getDTM((Source)null, true, this.m_wsfilter, true, false, false, initSize, this.m_buildIdIndex);
      }
   }
   ...
   public final class NamespaceChildrenIterator extends DTMDefaultBaseIterators.InternalAxisIteratorBase {
      ...
      public int next() {
         if (this._currentNode != -1) {
            for(int node = -2 == this._currentNode ? SAXImpl.this._firstch(SAXImpl.this.makeNodeIdentity(this._startNode)) : SAXImpl.this._nextsib(this._currentNode); node != -1; node = SAXImpl.this._nextsib(node)) {
