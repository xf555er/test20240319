...
public abstract class SyntaxTreeNode implements Constants {
   ...
   protected final void parseChildren(Parser parser) {
      ...
      int nLocals;
      for(nLocals = 0; nLocals < count; ++nLocals) {
         SyntaxTreeNode child = (SyntaxTreeNode)this._contents.elementAt(nLocals);
   ...
   protected final int getNodeIDForStylesheetNSLookup() {
      if (this._nodeIDForStylesheetNSLookup == -1) {
         ...
         if (prefixMapping == null) {
            this._nodeIDForStylesheetNSLookup = parentNodeID;
         } else {
            this._nodeIDForStylesheetNSLookup = this.getXSLTC().registerStylesheetPrefixMappingForRuntime(prefixMapping, parentNodeID);
         }
      }
