      ...
      if (!this.isStandard() && !this.isExtension()) {
         if (this.unresolvedExternal) {
            ...
            if (this._isExtConstructor) {
               ...
               for(i = 0; i < n; ++i) {
                  arg = this.argument(i);
                  Type expType = arg.getType();
                  arg.translate(classGen, methodGen);
                  arg.startIterator(classGen, methodGen);
                  expType.translateTo(classGen, methodGen, paramTypes[i]);
                  paramTemp[i] = methodGen.addLocalVariable("function_call_tmp" + i, expType.toJCType(), (InstructionHandle)null, (InstructionHandle)null);
         ...
         clazz = this._fname.toString().replace('-', '_') + "F";
         String args = "";
         if (clazz.equals("sumF")) {
            args = "Lorg/apache/xalan/xsltc/DOM;";
            il.append(methodGen.loadDOM());
         } else if (clazz.equals("normalize_spaceF") && this._chosenMethodType.toSignature(args).equals("()Ljava/lang/String;")) {
            args = "ILorg/apache/xalan/xsltc/DOM;";
            il.append(methodGen.loadContextNode());
            il.append(methodGen.loadDOM());
         }

         index = cpg.addMethodref("org.apache.xalan.xsltc.runtime.BasisLibrary", clazz, this._chosenMethodType.toSignature(args));
         il.append((org.apache.bcel.generic.Instruction)(new INVOKESTATIC(index)));
      }
