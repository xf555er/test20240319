...
public class SAX2DTM extends DTMDefaultBaseIterators implements EntityResolver, DTDHandler, ContentHandler, ErrorHandler, DeclHandler, LexicalHandler {
   ...
   public String getNodeName(int nodeHandle) {
      ...
      if (0 == namespaceID) {
         ...
         if (type == 13) {
            return null == this.m_expandedNameTable.getLocalName(expandedTypeID) ? "xmlns" : "xmlns:" + this.m_expandedNameTable.getLocalName(expandedTypeID);
         } else {
            return 0 == this.m_expandedNameTable.getLocalNameID(expandedTypeID) ? m_fixednames[type] : this.m_expandedNameTable.getLocalName(expandedTypeID);
         }
      } else {
   ...
   public XMLString getStringValue(int nodeHandle) {
      ...
      if (this.isTextType(type)) {
         ...
         if (-1 != firstChild) {
            ...
            return this.m_xstrf.newstr(this.m_valuesOrPrefixes.indexToString(offset));
         }
   ...
   public void processingInstruction(String target, String data) throws SAXException {
      this.charactersFlush();
      int exName = this.m_expandedNameTable.getExpandedTypeID((String)null, target, 7);
