...
public class ProcessInject {
   ...
   public void checkExecuteList() {
      ...
      boolean var2 = false;
      Iterator var3 = this.getExecuteList().iterator();

      while(true) {
         while(var3.hasNext()) {
            String var4 = (String)var3.next();
            if (!"CreateThread".equals(var4) && !var4.startsWith("CreateThread ")) {
               if ("SetThreadContext".equals(var4)) {
                  var1.add("x64 -> x86 (suspended)");
                  var1.add("x64 -> x64 (suspended)");
                  var1.add("x86 -> x86 (suspended)");
               } else if ("CreateRemoteThread".equals(var4)) {
                  var1.add("x86 -> x86");
                  var1.add("x64 -> x86");
                  var1.add("x64 -> x64");
                  ...
                  var1.add("x86 -> x86 (suspended)");
                  var2 = true;
               } else if ("RtlCreateUserThread".equals(var4)) {
                  var1.add("x64 -> x86");
                  var1.add("x64 -> x64");
                  var1.add("x86 -> x64");
                  var1.add("x86 -> x86");
                  ...
                  var1.add("self-inject");
                  if (!var2) {
                     this.warnings.add(".process-inject.execute " + var4 + " will cause unpredictable behavior with cross-session injects on XP/2003. Specify CreateRemoteThread, above this function, to mitigate.");
                  }
               } else if ("NtQueueApcThread".equals(var4)) {
                  var1.add("x86 -> x86");
                  var1.add("x86 -> x86 (cross session)");
                  var1.add("x64 -> x64");
                  var1.add("x64 -> x64 (cross session)");
               } else if (var4.startsWith("CreateRemoteThread ")) {
                  var1.add("x86 -> x86");
                  var1.add("x64 -> x64");
                  var1.add("self-inject");
                  var1.add("x64 -> x64 (suspended)");
                  var1.add("x86 -> x86 (suspended)");
                  if (!CommonUtils.isin(" ntdll", var4.toLowerCase()) && !CommonUtils.isin(" kernel32", var4.toLowerCase())) {
                     this.warnings.add(".process-injext.execute " + var4 + " should reference an ntdll or kernel32 function.");
                  }
               } else if ("NtQueueApcThread-s".equals(var4)) {
                  var1.add("x64 -> x64 (suspended)");
                  var1.add("x86 -> x86 (suspended)");
               }
            } else {
         ...
         byte[] var5 = this.getExecuteValue();
         if (var5.length > 128) {
            this.errors.add(".process-inject.execute block is " + var5.length + " bytes. Reduce its size to <128b or Beacon will crash.");
         }
