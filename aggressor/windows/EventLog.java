...
public class EventLog extends AObject implements ActionListener, ConsolePopup, Callback, Do {
   ...
   public void actionPerformed(ActionEvent var1) {
      ...
      try {
         ...
         this.console.append(var6.getMessage() + "\n");
         return;
      }

      this.console.addCommandToHistory(var2);
      CommandParser var7 = new CommandParser(var2);
      if (var7.is("/msg")) {
         if (var7.verify("AZ")) {
            String var4 = var7.popString();
            String var5 = var7.popString();
            this.conn.call("aggressor.event", CommonUtils.args(LoggedEvent.Private(this.nick, var5, var4)), (Callback)null);
         }
      } else if (!var7.is("/names") && !var7.is("/sc")) {
         if (var7.is("/me")) {
            if (var7.verify("Z")) {
               this.conn.call("aggressor.event", CommonUtils.args(LoggedEvent.Action(this.nick, var7.popString())), (Callback)null);
            }
         } else {
            if (var7.is("history")) {
               int var9 = var7.popHistoryArgument();
               if (var7.hasError()) {
                  this.console.append(var7.error() + "\n");
               } else {
         ...
         LinkedList var8 = new LinkedList(DataUtils.getUsers(this.data));
         Collections.sort(var8);
         Stack var10 = new Stack();
         var10.push(SleepUtils.getArrayWrapper(var8));
         this.console.append(this.engine.format("EVENT_USERS", var10) + "\n");
      }
