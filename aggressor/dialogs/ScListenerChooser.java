...
public class ScListenerChooser extends AObject implements Callback, ActionListener {
   ...
   public void show() {
      ...
      if (this.behavior == 1) {
         if (DataUtils.hostStage(this.client.getData())) {
            this.dialog.add(DialogUtils.description("This feature requires a payload stager. Only payloads with hosted payload stages (e.g., HTTP/S and DNS Beacons) are shown here."), "North");
         } else {
            this.dialog.add(DialogUtils.description("This feature requires a payload stager. Only payloads with hosted payload stages are shown here. .host_stage is false in your profile. Expect fewer options."), "North");
         }
      }
